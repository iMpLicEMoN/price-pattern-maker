...
export const setBar = createEvent('setBar')
export const saveChart = createEvent('saveChart')
export const saveSeries = createEvent('saveSeries')

export const chart = createStore({})
    .on(saveChart, (state, chart) => chart)
export const candlestickSeries = createStore({})
    .on(saveSeries, (state, series) => series)

export const chartData = createStore([
    ...
    .on(setBar, (state, newBar) => {
        const newState = state.map(bar => {
            let oldBarTime = bar.time
            if (bar.time.day)
                oldBarTime=moment(bar.time.year+"-"+bar.time.month+"-"+bar.time.day).format('YYYY-MM-DD')

            let newBarTime = newBar.time
            if (newBar.time.day)
                newBarTime=moment(newBar.time.year+"-"+newBar.time.month+"-"+newBar.time.day).format('YYYY-MM-DD')
